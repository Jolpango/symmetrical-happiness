...
\section{Space}
The most direct way to make spaces is simply to use the % s hit pasd
...
\LaTeX\ also has a number of predefined spaces.
To produce a
space with fixed width, and which cannot be used as a line break,
you may use the \verb(~( character.
...
\verb(\qquad(.
There is also a ``thin space'': \verb(\,(.
...
use to push text around.
For example, the following line is 
set using the command \verb(text\hfill text(.
The line below it 
...
If more than one \verb(\hfill( appears
...
command.
The framebox command puts a box around its argument,
so that \verb(\framebox{text}( looks like \framebox{text}.
It takes
...
all even-numbered pages 0.
5 inches wider than the default (which is 
1 inch), we would define\\
\verb(\evensidemargin=0.
5in(
.
\\
...
\begin{figure}[ht]
    \begin{center}
    \includegraphics[width=4in]{sniper.
    eps}
